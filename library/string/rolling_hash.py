...
class RollingHash:
    ...
    def __init__(self, string: str) -> None:
        ...
        for char in string:
            self._hash.append(
                self._calc_mod(self._mul(self._hash[-1], RollingHash.base) + ord(char))
            )
    ...
    def _make_base_pow(self, x: int) -> None:
        assert RollingHash.base
        l = len(RollingHash.base_pow)
        for _ in range(x - l + 1):
            RollingHash.base_pow.append(
                self._mul(RollingHash.base, RollingHash.base_pow[-1])
            )
    ...
    def join(hash_s: int, hash_t: int, len_t: int) -> int:
        ...
        return (
            (hash_s * RollingHash.base_pow[len_t]) % RollingHash.MOD + hash_t
        ) % RollingHash.MOD
