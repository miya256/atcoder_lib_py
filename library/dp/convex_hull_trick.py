...
class ConvexHullTrick:
    ...
    def _add(self, line: "ConvexHullTrick.Line") -> None:
        ...
        if 0 < position < len(self._lines) and self._is_bad(
            self._lines[position - 1], line, self._lines[position]
        ):
            return
        # 右側の不要な直線を削除
        while position + 1 < len(self._lines) and self._is_bad(
            line, self._lines[position], self._lines[position + 1]
        ):
            self._lines.pop(position)
        # 左側の不要な直線を削除
        while position - 2 >= 0 and self._is_bad(
            self._lines[position - 2], self._lines[position - 1], line
        ):
    ...
    def _add_left(self, line: "ConvexHullTrick.Line") -> None:
        while len(self._lines) >= 2 and self._is_bad(
            line, self._lines[0], self._lines[1]
        ):
            self._lines.popleft()
    ...
    def _query_from_left(self, x: int) -> int:
        while len(self._lines) > 1 and self._compare(
            self._lines[0](x), self._lines[1](x)
        ):
            self._lines.popleft()
    ...
    def _query_from_right(self, x: int) -> int:
        while len(self._lines) > 1 and self._compare(
            self._lines[-1](x), self._lines[-2](x)
        ):
            self._lines.pop()
