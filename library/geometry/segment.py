...
class Segment:
    ...
    def intersects(self, other) -> bool:
        ...
        if (
            max(self.p.x, self.q.x) < min(other.p.x, other.q.x)
            or max(other.p.x, other.q.x) < min(self.p.x, self.q.x)
            or max(self.p.y, self.q.y) < min(other.p.y, other.q.y)
            or max(other.p.y, other.q.y) < min(self.p.y, self.q.y)
        ):
            return False
